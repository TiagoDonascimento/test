...
puts "1. Each username followed by its description. If there is no description type nil."
puts ""
     twitter_data.each do |key,value|
       key.each do |look|
            username = look[0]
    description = look[1]["description"]

  if
    look[1]["description"] == nil
    puts "#{username}: nil"
  else
...
puts ""
  ...
  twitter_data.each do |key,value|
    key.each do |look|
         username = look[0]
  description = look[1]["number of followers"]
  puts "#{username}: #{description}"
end
end
puts ""
...
description = look[1]["latest tweet"].length
puts "#{username}'s latest tweet is #{description} characters long."
end
end
...
totally = 0
 ...
 twitter_data.each do |loopy23|
   loopy23.each do |again3|
      username23 = again3[0]#username
      again3[1]["last twenty tweets"].each do |loopus|
        counterize += 1
 totally += loopus.length

if
counterize == 20
puts "#{username23} used #{totally} characters in their last twenty tweets."
...
twitter_names = Array.new

 twitter_data.each do |loopy|
   loopy.each do |again|
      username = again[0]#username
  collection << again[1]["number of followers"]
 ...
 corner = Hash[twitter_names.zip(collection)]

max_quantity = corner.values.max
max_names = corner.select { |k, v| v == max_quantity }.keys



puts "#{max_names} has the most followers."
...
twitter_names2 = Array.new

 twitter_data.each do |loopy2|
   loopy2.each do |again|
      username2 = again[0]#username
  collection2 << again[1]["number of friends"]
  twitter_names2 <<    username2
 ...
 corner2 = Hash[twitter_names2.zip(collection2)]

max_quantity2 = corner2.values.max
max_names2 = corner2.select { |k, v| v == max_quantity2 }.keys



puts "#{max_names2} has the most friends."
...
collection3 = Array.new
twitter_names3 = Array.new

 twitter_data.each do |loopy3|
   loopy3.each do |again|
      username3 = again[0]#username
  collection3 << again[1]["number of tweets"]
  twitter_names3 <<    username3
 ...
 corner3 = Hash[twitter_names3.zip(collection3)]

max_quantity3 = corner3.values.max
max_names3 = corner3.select { |k, v| v == max_quantity3 }.keys



puts "#{max_names3} has the greatest number of tweets.\n"
puts ""
puts "Names of those who have a description are:\n"
puts ""
twitter_data.each do |loopinize|
  loopinize.each do |looper|
    if
    non_nil  = looper[1]["description"] != nil
...
puts "Names of those who have location in twitter data are:\n"
puts ""
twitter_data.each do |loopinize2|
  loopinize2.each do |looper2|
    if
    non_nil2  = looper2[1]["location"] != nil
